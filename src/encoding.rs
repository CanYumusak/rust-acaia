...
pub async fn request_settings(device: &dyn Peripheral, cmd_char: &Characteristic) {
...
pub async fn tare(device: &dyn Peripheral, cmd_char: &Characteristic) {
    let payload = &[0];
...
pub async fn start_timer(device: &dyn Peripheral, cmd_char: &Characteristic) {
    let payload = &[0, 0];
    return send_action(device, cmd_char, payload);
}

pub async fn stop_timer(device: &dyn Peripheral, cmd_char: &Characteristic) {
    let payload = &[0, 2];
    return send_action(device, cmd_char, payload);
}

pub async fn reset_timer(device: &dyn Peripheral, cmd_char: &Characteristic) {
    let payload = &[0, 1];
    return send_action(device, cmd_char, payload);
}

async fn send_action(device: &dyn Peripheral, cmd_char: &Characteristic, payload: &[u8]) {
    return device.write(cmd_char, &*encode(13, payload), WriteType::WithoutResponse)
        .await
        .unwrap();
}

pub async fn ident(device: &dyn Peripheral, cmd_char: &Characteristic) {
...
pub async fn request_heartbeat(device: &dyn Peripheral, cmd_char: &Characteristic) {
